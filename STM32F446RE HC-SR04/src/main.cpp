...
const uint8_t LED_PWM = 3;     // Pin yang mendukung PWM
const uint8_t LED_FAR = 4;     // Pin Digital biasa
...
void setup() {
  ...
  // Menggunakan resolusi 12-bit untuk transisi PWM yang sangat halus
...
void loop() {
  ...
  // Kondisi 2: Objek berada di jarak <= 30 cm
    ...
    targetPwmValue = map(cm, 0, THRESHOLD_DISTANCE, 4095, 0);
    targetPwmValue = constrain(targetPwmValue, 0, 4095);
